...
import { Building2, UserCheck, Server, Plus, Trash2, AlertCircle, CheckCircle2 } from 'lucide-react';
...
/**
 * 「組織清單管理」頁面：維護 departments / project_managers / deployment_environments 三張對照表，
 * 供「專案管理」的「新增專案」表單以下拉選單方式讀取所屬部門與專案經理，以及「使用產品清單」的
 * 產品版本套用表單讀取可選之部署環境（例如 DEV/SIT/UAT/PRD），取代原本的自由輸入文字或固定清單。
 */
export const OrgDirectoryManager: React.FC = () => {
  ...
  const [deploymentEnvironments, setDeploymentEnvironments] = useState<OrgEntry[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      setDeploymentEnvironments(data.deploymentEnvironments || []);
    } catch (err: any) {
  ...
  const handleAddDeploymentEnvironment = async () => {
    if (!newEnvName.trim()) return;
    setSavingEnv(true);
    setError(null);
    try {
      const res = await fetch('/api/org-directory/environments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `新增失敗 (HTTP ${res.status})`);
      setNotice(`已新增部署環境「${newEnvName.trim()}」。`);
      setNewEnvName('');
      await load();
    } catch (err: any) {
      setError(err.message || '新增部署環境失敗');
    } finally {
      setSavingEnv(false);
    }
  };

  const handleDeleteDeploymentEnvironment = async (entry: OrgEntry) => {
    if (!window.confirm(`確定要刪除部署環境「${entry.name}」嗎？`)) return;
    setError(null);
    try {
      const res = await fetch(`/api/org-directory/environments/${encodeURIComponent(entry.id)}`, { method: 'DELETE' });
      if (!res.ok) throw new Error(`刪除失敗 (HTTP ${res.status})`);
      await load();
    } catch (err: any) {
      setError(err.message || '刪除部署環境失敗');
    }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div className="border border-slate-200 rounded-2xl overflow-hidden">
          <div className="bg-slate-50 border-b border-slate-200 p-4 space-y-3">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleAddDeploymentEnvironment()}
                placeholder="輸入部署環境代號，例如 DEV"
                className="flex-1 bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 font-medium focus:outline-none focus:border-blue-500 shadow-2xs"
              />
              <button
                type="button"
                disabled={savingEnv || !newEnvName.trim()}
                onClick={handleAddDeploymentEnvironment}
                className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs flex items-center space-x-1.5 shadow-sm transition-all disabled:opacity-50 shrink-0"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>新增</span>
          ...
          <div className="divide-y divide-slate-100">
            {loading && <div className="px-4 py-6 text-center text-slate-400 text-xs">載入中...</div>}
            {!loading && deploymentEnvironments.length === 0 && (
              <div className="px-4 py-6 text-center text-slate-400 text-xs">尚無部署環境資料，請於上方新增。</div>
            )}
            {!loading &&
              deploymentEnvironments.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-slate-50/60">
                  <span className="text-xs font-bold text-slate-900">{entry.name}</span>
                  <button
                    type="button"
                    onClick={() => handleDeleteDeploymentEnvironment(entry)}
                    className="p-1.5 rounded-lg hover:bg-red-50 text-red-600"
                    title="刪除"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
